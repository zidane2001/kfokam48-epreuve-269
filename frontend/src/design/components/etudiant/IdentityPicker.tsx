...
import { authRequise } from '../../auth/authStore';
import type { Identite } from '../../types/domain';

/**
 * Point d'entree etudiant. Mode auth (evolution PO) : passerelle de connexion.
 * Mode AUTH_REQUIS=false (contrat du sujet, Q1) : choix libre d'identite, design
 * d'origine intact.
 */
export function IdentityPicker({ onSelect }: { onSelect: (identite: Identite) => void; }) {
  ...
  const etudiantsQ = useApiQuery(() => api.listerEtudiants(promotionId), [promotionId], {
    enabled: Boolean(promotionId) && !authRequise()
  });
